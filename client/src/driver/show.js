   ...
   state = {web3:null,accounts:null,driverContract:null,ar:[]}
    ...
    componentDidMount = async () => {
      ...
      const driverInstance = new web3.eth.Contract(
        ...
                    
      const arr = await driverInstance.methods.returnDriver(accounts[0]).call()
      console.log(arr)
      //let some = []
      //arr.map(d=>{
        //  d.map(i=>{
    ...
    if(this.state.ar.length===0){
           return <><Header who="driver"/><div style={{textAlign:'center'}}><h1 className="title is-1">No transactions as you don't completed any ride.</h1></div></>
    }
